...
for head in HEADS:
    ...
    for i in range(6):
        ...
        output_img = cv.resize(output_img, (2000, 2000), interpolation=cv.INTER_CUBIC)
        #Image.fromarray(output_img, mode='RGB').save(os.path.join(result_dir, head + '_Hour_' + str(pre_tid) + '.jpg'))
