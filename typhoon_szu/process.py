...
import os, time, scipy.io, shutil, scipy
import numpy as np
import cv2 as cv
...
def filling(process_dir, HEADS):
    for head in HEADS:
        ...
        for cid in range(first_tid, last_tid + 1):
            ...
            # check data loss
            data1 = np.load(os.path.join(process_dir, head + '_Hour_' + str(cid) + '_Band_08.npy')) / 40950.0
            ...
            if sum(sum(data1 >= 1)) > 40960 or sum(sum(data1 <= 0)) > 40960:
                print(name1)
                data1 = np.load(os.path.join(process_dir, head + '_Hour_' + str(cid - 1) + '_Band_08.npy'))
                np.save(os.path.join(name1), data1)
...
def save2img(process_dir, output_dir, HEADS, ps):
    ...
    for head in HEADS:
        ...
        for cid in range(first_tid, last_tid + 1):
            ...
            data1 = np.expand_dims(np.load(name1), axis=2) / 40950.0
            data2 = np.expand_dims(np.load(name2), axis=2) / 4095.0
            data3 = np.expand_dims(np.load(name3), axis=2) / 4095.0
            data = np.clip(np.concatenate((data1, data2, data3), axis=2), 0, 1)

            img = cv.resize(data, (ps, ps), interpolation=cv.INTER_CUBIC)
            img = Image.fromarray(img, mode='RGB').save(os.path.join(output_dir, head + '_Hour_' + str(cid) + '.png'))
